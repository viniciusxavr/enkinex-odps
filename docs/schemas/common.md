...
| `Description` (`common/description.k`) | `$defs.Description` | Inherits `Extensible`; upstream has no `tags` on `Description`, matched exactly |
...
| `Tags` (`common/base.k`) | `$defs.Tags` | 1:1: `type Tags = [str]` |
| `Extensible` (`common/base.k`) | no upstream equivalent | Synthesized base schema: see below |
| `Taggable` (`common/base.k`) | no upstream equivalent | Synthesized base schema: see below |

## Architecture Decisions

- `Extensible`/`Taggable` were introduced to eliminate the `tags`/`customProperties`/`authoritativeDefinitions` trio that was duplicated across `InputPort`, `OutputPort`, `ManagementPort`, `Support`, `TeamMember`, `Team`, and the root `DataProduct`. JSON Schema has no inheritance mechanism, so upstream repeats this trio in every `$defs` entry that needs it; KCL does, so this port doesn't have to.
- Split into two schemas, not one, because upstream's own entities aren't uniform: `$defs.Description` has `customProperties` + `authoritativeDefinitions` but deliberately **no** `tags`. Rather than give `Description` an unused `tags` field, or duplicate the pair a second time, `Extensible` holds the pair and `Taggable(Extensible)` layers `tags` on top: every consumer inherits from whichever base matches its actual upstream shape.
...
- If KCL gains structural/duck-typed schema composition (satisfying a shape without nominal inheritance), the `Extensible`/`Taggable` split could potentially collapse into a single schema with an optional `tags` field: revisit if that lands.
