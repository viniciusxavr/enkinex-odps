...
- Inherits `common.TagsDiscoverable` (see [[common]]). `url` is required (matches upstream's `required: [channel, url]`) and always validated; `invitationUrl` is optional and validated only when present.
