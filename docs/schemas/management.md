...
- Same open item as [[odps]] and [[support]]: if `content`/`$type` should ever become closed unions, do it consistently with the other `examples`-only fields across the port, not in isolation.
