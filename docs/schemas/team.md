...
|---|---|---|
| `TeamMember` (`team/member.k`) | `$defs.TeamMember` | Inherits `common.Taggable`; adds its own `check` for `dateIn`/`dateOut` |
| `Team` (`team/team.k`) | `$defs.Team` | Inherits `common.Taggable`; `members: [TeamMember]` |

## Architecture Decisions

- `TeamMember` keeps its own `check` block for `dateIn`/`dateOut` rather than folding date validation into a shared base, because `Extensible`/`Taggable` (see [[common]]) only model the trio that's identical across *every* consumer. `dateIn`/`dateOut` are unique to `TeamMember` and don't belong in a shared base that only one schema uses.
