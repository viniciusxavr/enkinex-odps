...
| `InputPort` (`product/input/port.k`) | `$defs.InputPort` | Inherits `common.Taggable` |
...
- `InputContract` is *not* `Taggable` (see [[common]]): upstream's `$defs.InputContract` is a bare `{id, version}` dependency pointer with no `tags`/`customProperties`/`authoritativeDefinitions`, so there's nothing to inherit.
