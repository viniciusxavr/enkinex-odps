...
- Not `TagsDiscoverable`/`AuthoritativeCustomizable` (see [[common]]): upstream's `$defs.SBOM` has only `type`/`url`, no `tags`/`customProperties`/`authoritativeDefinitions`, so there's nothing to inherit.
